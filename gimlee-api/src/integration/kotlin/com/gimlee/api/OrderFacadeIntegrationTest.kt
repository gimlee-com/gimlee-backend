...
class OrderFacadeIntegrationTest(
    ...
    Given("a seller and an active Ad") {
        ...
        And("a buyer") {
            ...
            When("the buyer places an order through the facade") {
                val request = PlaceOrderRequestDto(adId = ad.id, amount = BigDecimal("10.00"), currency = Currency.ARRR)
                ...
                val response = objectMapper.readValue(result.response.contentAsString, OrderResponseDto::class.java)

                Then("the response should contain payment details") {
                    response.orderId shouldNotBe null
                    response.status shouldBe OrderStatus.AWAITING_PAYMENT.name
                ...
                And("the buyer checks the order status") {
                    val statusResult = mockMvc.get("/orders/${response.orderId}/status") {
                        requestAttr("principal", principal)
                    }.andExpect {
                        status { isOk() }
                    }.andReturn()

                    val statusResponse = objectMapper.readValue(statusResult.response.contentAsString, OrderStatusResponseDto::class.java)

                    Then("the status should be AWAITING_PAYMENT") {
                        statusResponse.orderId shouldBe response.orderId
                        statusResponse.status shouldBe OrderStatus.AWAITING_PAYMENT.name
                        statusResponse.paymentStatus shouldBe "AWAITING_CONFIRMATION"
                    }
                }
            }

            When("the buyer attempts to place an order with a mismatched amount") {
                val maliciousRequest = PlaceOrderRequestDto(adId = ad.id, amount = BigDecimal("1.00"), currency = Currency.ARRR)
